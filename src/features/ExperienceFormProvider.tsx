...
const schema = yup.object().shape({
  ...
  endYYYY: yearSchema.test('dateRules', '종료 기간은 시작 기간과 같거나 이후여야 합니다', (value, context) => {
    const startYYYY = context.resolve(yup.ref<string>('startYYYY'));
    if (!value?.trim() || !startYYYY) return;
    return value.length === 4 && startYYYY.length === 4 && value >= startYYYY;
  }),
  endMM: monthSchema.test('dateRules', '종료 기간은 시작 기간과 같거나 이후여야 합니다', (value, context) => {
    const startMM = context.resolve(yup.ref<string>('startMM'));
    if (!value?.trim() || !startMM) return;
    return value.length === 2 && startMM.length === 2 && value >= startMM;
  }),
  endDate: yup.string().required(),

  keywords: yup.array<KeywordEntriesType>().required(),
  situation: yup.string().required(),
  task: yup.string().required(),
  action: yup.string().required(),
  result: yup.string().required(),
  experienceStatus: yup.string().required(),
  writeStatus: yup.array<WriteStatusType[]>().optional(),
  capabilities: yup.array<CapabilitiesType[]>().required(),
